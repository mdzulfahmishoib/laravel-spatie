        ...
        <div class="card-body p-0">
            <div class="row p-4">
                <div class="col">
                    <div class="align-items-center text-center">
                        <img id="preview" src="{{ $user->foto ? asset('storage/management_user/foto_profil/' . $user->foto) : asset('img/default_user.jpg') }}" alt="Foto Profil" width="150" class="mt-3 img-circle elevation-2">
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col">
                            <div style="background-color: #E9ECEF;" class="p-3 rounded text-center">
                                <h6 class="text-bold">Kantor</h6>
                                <p class="mb-0" id="show_kantor"></p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col">
                    ...
                    <div class="mb-3">
                        ...
                        <input for="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" placeholder="Masukkan Nama" required value="{{ old('name', $user->name) }}" autofocus>
                        @error('name') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                    <div class="mb-3">
                        <label for="">Username<span class="text-danger"> *</span></label>
                        <input type="text" name="username" id="username" class="form-control @error('username') is-invalid @enderror" placeholder="Masukkan Username" required value="{{ old('username', $user->username) }}">
                        @error('username') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                    <div class="mb-3">
                        <label for="">Email<span class="text-danger"> *</span></label>
                        <input for="text" name="email" id="email" class="form-control @error('email') is-invalid @enderror" placeholder="Masukkan Email" required value="{{ old('email', $user->email) }}">
                        @error('email') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                    <div class="mb-3">
                        <label for="kantor_id">Kantor<span class="text-danger"> *</span></label>
                        <select name="kantor_id" id="kantor_id" class="form-control @error('kantor_id') is-invalid @enderror" required>
                            <option value="" selected disabled>Silakan pilih kantor</option>
                                @foreach ($kantor as $key => $item)
                                    <option value="{{ $key }}" {{ old('kantor_id', $user->kantor_id) == $key ? 'selected' : '' }}>{{ $item }}</option>
                                @endforeach
                        </select>
                        @error('kantor_id') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                    <div class="mb-3">
            ...
            <div class="card-footer">
                <div class="float-right">
                    <a href="{{ url('management_user/users') }}" class="btn btn-danger btn-sm"><i class="fas fa-times"></i> Kembali</a>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const showKantor = document.getElementById("show_kantor");
        ...
        function updateDisplay() {
            ...
            showKantor.textContent = kantorSelect.options[kantorSelect.selectedIndex]?.text || '';
        ...
        kantorSelect.addEventListener("change", updateDisplay);
